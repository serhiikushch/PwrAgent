...
5. **Authorization.** Apply the same `authorizedActorIds` allowlist to slash commands as to inbound text events — an unauthorized user typing `/resume` shouldn't bind anything. Return an ephemeral "not authorized" reply if your platform supports it; otherwise silently drop with a log.
