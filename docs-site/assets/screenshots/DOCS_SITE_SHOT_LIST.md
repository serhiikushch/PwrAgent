...
| `desktop-skills-autocomplete.png` | [/desktop/#skills-browser--autocomplete-and-chips](../../../docs-site/desktop.md) | Composer with `$ce` typed → Skills autocomplete listbox visible | Reuse [`skill-autocomplete-interactions` replay fixture](../../../apps/desktop/e2e/fixtures/skill-autocomplete-interactions/replay.fixture.json) — already ships realistic skills (`ce:plan`, `ce:brainstorm`, `ce:compound`, `adversarial-document-reviewer`, …). Test: `desktop-skills-autocomplete` in `docs-site-screenshots.inspect.spec.ts`. |
| `desktop-queued-turns.png` | [/desktop/#multi-message-queueing](../../../docs-site/desktop.md) | Composer with one in-flight turn and `/review` + a follow-up queued as chips | Inline fixture created by the test — one thread, `turn/start` + `turn/started` notification fired, then `/review` and `now squash and push --force-with-lease` queued via the Queue button. Test: `desktop-queued-turns` in `docs-site-screenshots.inspect.spec.ts`. |
